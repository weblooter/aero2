...
class PersonalSupportComponent extends \Local\Core\Inner\BxModified\CBitrixComponent
{
    ...
    protected function fillResult()
    {
        ...
        if( $this->arParams['SUPPORT_ID'] > 0 && in_array($this->arParams['SUPPORT_ID'], array_keys($arResult['SUPPORT_LIST'])) )
        {
            ...
            foreach ($arResult['SUPPORT_MSG'] as $arItem)
            {
                if( $arItem['ACTIVE'] == 'N' )
                {
                    \Local\Core\Model\Data\SupportMessageTable::update($arItem['ID'], ['ACTIVE' => 'Y']);
                }
            }
        }
