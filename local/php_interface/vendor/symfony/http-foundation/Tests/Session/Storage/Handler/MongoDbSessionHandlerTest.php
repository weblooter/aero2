    ...
    public function testRead()
    {
        ...
        $collection->expects( $this->once() )
            ...
            ->will( $this->returnCallback( function ( $criteria ) use ( $testTimeout ) {
                ...
                $this->assertInstanceOf( \MongoDB\BSON\UTCDateTime::class,
                    $criteria[ $this->options[ 'expiry_field' ] ][ '$gte' ] );
                $this->assertGreaterThanOrEqual( round( (string)$criteria[ $this->options[ 'expiry_field' ] ][ '$gte' ] / 1000 ),
                    $testTimeout );

                return [
                    $this->options[ 'id_field' ] => 'foo',
                    $this->options[ 'expiry_field' ] => new \MongoDB\BSON\UTCDateTime(),
                    $this->options[ 'data_field' ] => new \MongoDB\BSON\Binary( 'bar',
        ...
        $collection->expects( $this->once() )
            ...
            ->will( $this->returnCallback( function ( $criteria, $updateData, $options ) {
                ...
                $this->assertGreaterThanOrEqual( $expectedExpiry,
                    round( (string)$data[ $this->options[ 'expiry_field' ] ] / 1000 ) );
            } ) );
    ...
    public function testGc()
    {
        ...
        $collection->expects( $this->once() )
            ...
            ->will( $this->returnCallback( function ( $criteria ) {
                ...
                $this->assertGreaterThanOrEqual( time() - 1,
                    round( (string)$criteria[ $this->options[ 'expiry_field' ] ][ '$lt' ] / 1000 ) );
            } ) );
