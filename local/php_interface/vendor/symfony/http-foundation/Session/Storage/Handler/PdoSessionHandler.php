...
class PdoSessionHandler extends AbstractSessionHandler
{
    ...
    private function buildDsnFromUrl( $dsnOrUrl )
    {
        ...
        switch ( $driver )
        {
            ...
            default:
                throw new \InvalidArgumentException( sprintf( 'The scheme "%s" is not supported by the PdoSessionHandler URL configuration. Pass a PDO DSN directly.',
                    $params[ 'scheme' ] ) );
        }
    }
    ...
    protected function doRead( $sessionId )
    {
        ...
        do
        {
            ...
            if ( !filter_var( ini_get( 'session.use_strict_mode' ),
                    FILTER_VALIDATE_BOOLEAN ) && self::LOCK_TRANSACTIONAL === $this->lockMode && 'sqlite' !== $this->driver )
            {
    ...
    private function getMergeStatement( string $sessionId, string $data, int $maxlifetime ): ?\PDOStatement
    {
        switch ( true )
        {
            ...
            case 'sqlite' === $this->driver:
                ...
                break;
            case 'pgsql' === $this->driver && version_compare( $this->pdo->getAttribute( \PDO::ATTR_SERVER_VERSION ),
                    '9.5', '>=' ):
                $mergeSql = "INSERT INTO $this->table ($this->idCol, $this->dataCol, $this->lifetimeCol, $this->timeCol) VALUES (:id, :data, :lifetime, :time) ".
