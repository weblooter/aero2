...
final class SourceContextProvider implements ContextProviderInterface
{
    ...
    public function getContext(): ?array
    {
                ...
                while ( ++$i < $this->limit )
                {
                    ...
                    elseif ( isset( $trace[ $i ][ 'object' ] ) && $trace[ $i ][ 'object' ] instanceof Template )
                    {
                        ...
                        if ( isset( $info[ $trace[ $i - 1 ][ 'line' ] ] ) )
                        {
                            ...
                            $file = method_exists( $template,
                                'getSourceContext' ) ? $template->getSourceContext()->getPath() : null;
                            ...
                            if ( $src )
                            {
                                ...
                                for ( $i = max( $line - 3, 1 ), $max = min( $line + 3,
                                    \count( $src ) ); $i <= $max; ++$i )
                                {
        ...
        if ( null !== $this->projectDir )
        {
            ...
            {
                $context[ 'file_relative' ] = ltrim( substr( $file, \strlen( $this->projectDir ) ),
                    \DIRECTORY_SEPARATOR );
            }
        }
