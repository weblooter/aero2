...
class ExceptionCaster
{
    ...
    public static function castTraceStub( TraceStub $trace, array $a, Stub $stub, $isNested )
    {
        ...
        for ( $j += $trace->numberingOffset - $i++; isset( $frames[ $i ] ); ++$i, --$j )
        {
            ...
            if ( isset( $f[ $prefix.'src' ] ) )
            {
                ...
                {
                    $frame->value[ 'arguments' ] = new ArgsStub( $f[ 'args' ],
            ...
            $a[ substr_replace( $label, sprintf( 'separator=%s&', $frame instanceof EnumStub ? ' ' : ':' ), 2,
                0 ) ] = $frame;
    ...
    public static function castFrameStub( FrameStub $frame, array $a, Stub $stub, $isNested )
    {
        ...
        if ( isset( $f[ 'file' ], $f[ 'line' ] ) )
        {
            ...
            {
                if ( preg_match( '/\((\d+)\)(?:\([\da-f]{32}\))? : (?:eval\(\)\'d code|runtime-created function)$/',
                    $f[ 'file' ], $match ) )
                {
                    $f[ 'file' ] = substr( $f[ 'file' ], 0, -\strlen( $match[ 0 ] ) );
                    $f[ 'line' ] = (int)$match[ 1 ];
                }
                $caller = isset( $f[ 'function' ] ) ? sprintf( 'in %s() on line %d',
                ...
                {
                    if ( !empty( $f[ 'class' ] ) && ( is_subclass_of( $f[ 'class' ],
                                'Twig\Template' ) || is_subclass_of( $f[ 'class' ],
                                'Twig_Template' ) ) && method_exists( $f[ 'class' ], 'getDebugInfo' ) )
                    {
                        $template = isset( $f[ 'object' ] ) ? $f[ 'object' ] : unserialize( sprintf( 'O:%d:"%s":0:{}',
                            \strlen( $f[ 'class' ] ), $f[ 'class' ] ) );

                        $ellipsis = 0;
                        $templateSrc = method_exists( $template,
                            'getSourceContext' ) ? $template->getSourceContext()->getCode() : ( method_exists( $template,
                            'getSource' ) ? $template->getSource() : '' );
                        $templateInfo = $template->getDebugInfo();
                        if ( isset( $templateInfo[ $f[ 'line' ] ] ) )
                        {
                            if ( !method_exists( $template,
                                    'getSourceContext' ) || !file_exists( $templatePath = $template->getSourceContext()->getPath() ) )
                            {
                                $templatePath = null;
                            }
                            if ( $templateSrc )
                            {
                                $src = self::extractSource( $templateSrc, $templateInfo[ $f[ 'line' ] ],
                                    self::$srcContext, $caller, 'twig', $templatePath );
    ...
    private static function filterExceptionArray( $xClass, array $a, $xPrefix, $filter )
    {
        ...
        if ( !( $filter & Caster::EXCLUDE_VERBOSE ) && $trace )
        {
            ...
            {
                self::traceUnshift( $trace, $xClass, $a[ Caster::PREFIX_PROTECTED.'file' ],
        ...
        if ( isset( $a[ Caster::PREFIX_PROTECTED.'message' ] ) && false !== strpos( $a[ Caster::PREFIX_PROTECTED.'message' ],
                "class@anonymous\0" ) )
        {
            $a[ Caster::PREFIX_PROTECTED.'message' ] = preg_replace_callback( '/class@anonymous\x00.*?\.php0x?[0-9a-fA-F]++/',
                function ( $m ) {
        ...
        if ( isset( $a[ Caster::PREFIX_PROTECTED.'file' ], $a[ Caster::PREFIX_PROTECTED.'line' ] ) )
        {
            $a[ Caster::PREFIX_PROTECTED.'file' ] = new LinkStub( $a[ Caster::PREFIX_PROTECTED.'file' ],
                $a[ Caster::PREFIX_PROTECTED.'line' ] );
        }
