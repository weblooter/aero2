...
class ContainerBuilderTest extends TestCase
{
    ...
    public function testDefinitions()
    {
        ...
        $this->assertEquals( $definitions, $builder->getDefinitions(),
            '->setDefinitions() sets the service definitions' );
        $this->assertTrue( $builder->hasDefinition( 'foo' ),
            '->hasDefinition() returns true if a service definition exists' );
        $this->assertFalse( $builder->hasDefinition( 'foobar' ),
            '->hasDefinition() returns false if a service definition does not exist' );

        $builder->setDefinition( 'foobar', $foo = new Definition( 'FooBarClass' ) );
        $this->assertEquals( $foo, $builder->getDefinition( 'foobar' ),
            '->getDefinition() returns a service definition if defined' );
        $this->assertSame( $builder->setDefinition( 'foobar', $foo = new Definition( 'FooBarClass' ) ), $foo,
            '->setDefinition() implements a fluid interface by returning the service reference' );

        $builder->addDefinitions( $defs = ['foobar' => new Definition( 'FooBarClass' )] );
        $this->assertEquals( array_merge( $definitions, $defs ), $builder->getDefinitions(),
        ...
        {
            $this->assertEquals( 'You have requested a non-existent service "baz".', $e->getMessage(),
    ...
    /**
     * @group legacy
     * @expectedDeprecation The "deprecated_foo" service is deprecated. You should stop using it, as it will soon be
     *     removed.
     */
    ...
    public function testRegister()
    {
        ...
        $this->assertInstanceOf( 'Symfony\Component\DependencyInjection\Definition', $builder->getDefinition( 'foo' ),
            '->register() returns the newly created Definition instance' );
    }
    ...
    public function testAutowire()
    {
        ...
        $this->assertTrue( $builder->getDefinition( 'foo' )->isAutowired(),
            '->autowire() creates autowired definitions' );
    }
    ...
    public function testGetReturnsNullIfServiceDoesNotExistAndInvalidReferenceIsUsed()
    {
        ...
        $this->assertNull( $builder->get( 'foo', ContainerInterface::NULL_ON_INVALID_REFERENCE ),
            '->get() returns null if the service does not exist and NULL_ON_INVALID_REFERENCE is passed as a second argument' );
    }
    ...
    public function testGetReturnsSameInstanceWhenServiceIsShared()
    {
        ...
        $this->assertTrue( $builder->get( 'bar' ) === $builder->get( 'bar' ),
            '->get() always returns the same instance if the service is shared' );
    }
    ...
    public function testGetCreatesServiceBasedOnDefinition()
    {
        ...
        $this->assertInternalType( 'object', $builder->get( 'foo' ),
            '->get() returns the service definition associated with the id' );
    }
    ...
    public function testRegisterDoesNotOverrideExistingService()
    {
        ...
        $this->assertSame( $bar, $builder->get( 'bar' ),
            '->get() returns the service associated with the id even if a definition has been defined' );
    }
     ...
     * @expectedExceptionMessage You have requested a synthetic service ("foo"). The DIC does not know how to construct
     *     this service.
     */
    ...
    public function testAliases()
    {
        ...
        $this->assertSame( $builder->get( 'bar' ), $builder->get( 'foo' ),
        ...
        {
            $this->assertEquals( 'An alias can not reference itself, got a circular reference on "foobar".',
                $e->getMessage(), '->setAlias() throws an InvalidArgumentException if the alias references itself' );
        }
        ...
        {
            $this->assertEquals( 'The service alias "foobar" does not exist.', $e->getMessage(),
                '->getAlias() throws an InvalidArgumentException if the alias does not exist' );
        }
    }
    ...
    public function testGetAliases()
    {
        ...
        $this->assertCount( 2, $builder->getAliases(),
            '->getAliases() does not return aliased services that have been overridden' );
    }
    ...
    public function testAddGetCompilerPass()
    {
        ...
        $builder->addCompilerPass( $pass1 = $this->getMockBuilder( 'Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface' )->getMock(),
            PassConfig::TYPE_BEFORE_OPTIMIZATION, -5 );
        $builder->addCompilerPass( $pass2 = $this->getMockBuilder( 'Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface' )->getMock(),
    ...
    public function testCreateService()
    {
        ...
        $this->assertInstanceOf( '\Bar\FooClass', $builder->get( 'foo1' ),
            '->createService() requires the file defined by the service definition' );
        $this->assertInstanceOf( '\Bar\FooClass', $builder->get( 'foo2' ),
            '->createService() replaces parameters in the file provided by the service definition' );
    }
    ...
    public function testCreateServiceClass()
    {
        ...
        $this->assertInstanceOf( '\stdClass', $builder->get( 'foo1' ),
            '->createService() replaces parameters in the class provided by the service definition' );
    }
    ...
    public function testCreateServiceArguments()
    {
        $builder = new ContainerBuilder();
        $builder->register( 'bar', 'stdClass' );
        $builder->register( 'foo1', 'Bar\FooClass' )->addArgument( [
            'foo' => '%value%',
            '%value%' => 'foo',
            new Reference( 'bar' ),
            '%%unescape_it%%'
        ...
        $this->assertEquals( ['foo' => 'bar', 'bar' => 'foo', $builder->get( 'bar' ), '%unescape_it%'],
            $builder->get( 'foo1' )->arguments,
            '->createService() replaces parameters and service references in the arguments provided by the service definition' );
    }
    ...
    public function testCreateServiceFactory()
    {
        ...
        $this->assertTrue( $builder->get( 'foo' )->called,
            '->createService() calls the factory method to create the service instance' );
        $this->assertTrue( $builder->get( 'qux' )->called,
            '->createService() calls the factory method to create the service instance' );
    ...
    public function testCreateServiceMethodCalls()
    {
        ...
        $builder->setParameter( 'value', 'bar' );
        $this->assertEquals( ['bar', $builder->get( 'bar' )], $builder->get( 'foo1' )->bar,
            '->createService() replaces the values in the method calls arguments' );
    }
    ...
    public function testCreateServiceMethodCallsWithEscapedParam()
    {
        ...
        $builder->setParameter( 'value', 'bar' );
        $this->assertEquals( ['%unescape_it%'], $builder->get( 'foo1' )->bar,
            '->createService() replaces the values in the method calls arguments' );
    }
    ...
    public function testCreateServiceProperties()
    {
        $builder = new ContainerBuilder();
        $builder->register( 'bar', 'stdClass' );
        $builder->register( 'foo1', 'Bar\FooClass' )->setProperty( 'bar',
            ['%value%', new Reference( 'bar' ), '%%unescape_it%%'] );
        $builder->setParameter( 'value', 'bar' );
        $this->assertEquals( ['bar', $builder->get( 'bar' ), '%unescape_it%'], $builder->get( 'foo1' )->bar,
            '->createService() replaces the values in the properties' );
    }
    ...
    public function testCreateServiceConfigurator()
    {
        ...
        $builder->register( 'foo4', 'Bar\FooClass' )->setConfigurator( [
            $builder->getDefinition( 'baz' ),
            'configure'
        ] );
        ...
        {
            $this->assertEquals( 'The configure callable for class "Bar\FooClass" is not a callable.', $e->getMessage(),
                '->createService() throws an InvalidArgumentException if the configure callable is not a valid callable' );
        }
    }
        ...
        $builder
            ...
            ->setArguments( [
                new IteratorArgument( [
                    'k1' => new Reference( 'bar' ),
                    new Reference( 'invalid', ContainerInterface::IGNORE_ON_INVALID_REFERENCE )
                ] ),
                new IteratorArgument( [] ),
            ] );
    ...
    public function testCreateServiceWithExpression()
    {
        ...
        $builder->register( 'foo',
            'Bar\FooClass' )->addArgument( ['foo' => new Expression( 'service("bar").foo ~ parameter("bar")' )] );
    ...
    public function testResolveServices()
    {
        $builder = new ContainerBuilder();
        $builder->register( 'foo', 'Bar\FooClass' );
        $this->assertEquals( $builder->get( 'foo' ), $builder->resolveServices( new Reference( 'foo' ) ),
            '->resolveServices() resolves service references to service instances' );
        $this->assertEquals( ['foo' => ['foo', $builder->get( 'foo' )]],
            $builder->resolveServices( ['foo' => ['foo', new Reference( 'foo' )]] ),
            '->resolveServices() resolves service references to service instances in nested arrays' );
        $this->assertEquals( $builder->get( 'foo' ), $builder->resolveServices( new Expression( 'service("foo")' ) ),
    ...
    public function testMerge()
    {
        ...
        $this->assertEquals( ['bar' => 'foo', 'foo' => 'bar'], $container->getParameterBag()->all(),
        ...
        $container->merge( $config );
        $container->compile();
        $this->assertEquals( ['bar' => 'foo', 'foo' => 'foo'], $container->getParameterBag()->all(),
            '->merge() evaluates the values of the parameters towards already defined ones' );
        ...
        $container->merge( $config );
        $container->compile();
        $this->assertEquals( ['bar' => 'foo', 'foo' => 'foo', 'baz' => 'foo'], $container->getParameterBag()->all(),
            '->merge() evaluates the values of the parameters towards already defined ones' );
        ...
        $container->merge( $config );
        $this->assertEquals( ['service_container', 'foo', 'bar', 'baz'], array_keys( $container->getDefinitions() ),
        ...
        $container->merge( $config );
        $this->assertEquals( 'BazClass', $container->getDefinition( 'foo' )->getClass(),
        ...
        $container->merge( $config );
        $this->assertSame( ['AInterface' => $childDefA, 'BInterface' => $childDefB],
            $container->getAutoconfiguredInstanceof() );
    }

    /**
     * @expectedException \Symfony\Component\DependencyInjection\Exception\InvalidArgumentException
     * @expectedExceptionMessage "AInterface" has already been autoconfigured and merge() does not support merging
     *     autoconfiguration for the same class/interface.
     */
    ...
    /**
     * @expectedException \Symfony\Component\DependencyInjection\Exception\RuntimeException
     * @expectedExceptionMessage A string value must be composed of strings and/or numbers, but found parameter
     *     "env(json:ARRAY)" of type array inside string value "ABC %env(json:ARRAY)%".
     */
     ...
     * @expectedExceptionMessage Circular reference detected for parameter "env(resolve:DUMMY_ENV_VAR)"
     *     ("env(resolve:DUMMY_ENV_VAR)" > "env(resolve:DUMMY_ENV_VAR)").
     */
    ...
    public function testfindTaggedServiceIds()
    {
        ...
        $this->assertEquals( [], $builder->findTaggedServiceIds( 'foobar' ),
            '->findTaggedServiceIds() returns an empty array if there is annotated services' );
    }
    ...
    public function testFindUnusedTags()
    {
        ...
        $this->assertEquals( ['kenrel.event_listener'], $builder->findUnusedTags(),
    ...
    public function testFindDefinition()
    {
        ...
        $this->assertEquals( $definition, $container->findDefinition( 'foobar' ),
    ...
    public function testResources()
    {
        ...
        $this->assertEquals( [$a, $b], $resources,
    ...
    public function testFileExists()
    {
        ...
        $this->assertEquals( [$A, $a, $b, $c], $resources,
            '->getResources() returns an array of resources read for the current configuration' );
    }
    ...
    public function testExtension()
    {
        ...
        $this->assertSame( $container->getExtension( 'project' ), $extension,
    ...
    /**
     * @expectedException \Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException
     * @expectedExceptionMessage Circular reference detected for service "app.test_class", path: "app.test_class ->
    ...
    public function testInitializePropertiesBeforeMethodCalls()
    {
        ...
        $this->assertTrue( $container->get( 'bar' )->callPassed(),
            '->compile() initializes properties before method calls' );
    }
    ...
    /**
     * @expectedException \Symfony\Component\DependencyInjection\Exception\RuntimeException
     * @expectedExceptionMessage The definition for "DateTime" has no class attribute, and appears to reference a class
     *     or interface in the global namespace.
     */
    ...
    /**
     * @expectedException \Symfony\Component\DependencyInjection\Exception\RuntimeException
     * @expectedExceptionMessage The definition for "\DateTime" has no class attribute, and appears to reference a
     *     class or interface in the global namespace.
     */
    ...
    /**
     * @expectedException \Symfony\Component\DependencyInjection\Exception\RuntimeException
     * @expectedExceptionMessage The definition for "\Symfony\Component\DependencyInjection\Tests\FooClass" has no
     *     class attribute, and appears to reference a class or interface. Please specify the class attribute
     *     explicitly or remove the leading backslash by renaming the service to
     *     "Symfony\Component\DependencyInjection\Tests\FooClass" to get rid of this error.
     */
    ...
    public function testServiceLocator()
    {
        ...
        $container->register( 'bar_service',
            'stdClass' )->setArguments( [new Reference( 'baz_service' )] )->setPublic( true );
    ...
    public function testCaseSensitivity()
    {
        ...
        $this->assertNotSame( $container->get( 'foo' ), $container->get( 'fOO' ),
            '->get() returns the service for the given id, case sensitively' );
        $this->assertSame( $container->get( 'fOO' )->Foo->foo, $container->get( 'foo' ),
            '->get() returns the service for the given id, case sensitively' );
    }
