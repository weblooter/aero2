    ...
    public function process( ContainerBuilder $container )
    {
        foreach ( $container->getDefinitions() as $id => $definition )
        {
            ...
            if ( !$definition->isAbstract() && !$definition->isSynthetic() && !$definition->getClass() )
            {
                ...
                {
                    throw new RuntimeException( sprintf( 'Please add the class to service "%s" even if it is constructed by a factory since we might need to add method calls based on compile-time checks.',
                        $id ) );
                }
            ...
            foreach ( $definition->getTags() as $name => $tags )
            {
                foreach ( $tags as $attributes )
                {
                    foreach ( $attributes as $attribute => $value )
                    {
                        ...
                        {
                            throw new RuntimeException( sprintf( 'A "tags" attribute must be of a scalar-type for service "%s", tag "%s", attribute "%s".',
                                $id, $name, $attribute ) );
                        }
                    }
                }
            ...
            if ( $definition->isPublic() && !$definition->isPrivate() )
            {
                ...
                if ( null !== $usedEnvs )
                {
                    throw new EnvParameterException( [$resolvedId], null,
                        'A service name ("%s") cannot contain dynamic values.' );
                }
            }
        }
        ...
        foreach ( $container->getAliases() as $id => $alias )
        {
            if ( $alias->isPublic() && !$alias->isPrivate() )
            {
                ...
                {
                    throw new EnvParameterException( [$resolvedId], null,
                        'An alias name ("%s") cannot contain dynamic values.' );
                }
            }
        }
    }
