...
class GraphvizDumper extends Dumper
{
    ...
    public function dump( array $options = [] )
    {
        ...
        return $this->container->resolveEnvPlaceholders( $this->startDot().$this->addNodes().$this->addEdges().$this->endDot(),
            '__ENV_%s__' );
    }
    ...
    private function addNodes(): string
    {
        ...
        foreach ( $this->nodes as $id => $node )
        {
            ...
            $code .= sprintf( "  node_%s [label=\"%s\\n%s\\n\", shape=%s%s];\n", $this->dotize( $id ),
                $id.( $aliases ? ' ('.implode( ', ', $aliases ).')' : '' ), $node[ 'class' ],
                $this->options[ 'node' ][ 'shape' ], $this->addAttributes( $node[ 'attributes' ] ) );
        }
        ...
        foreach ( $this->edges as $id => $edges )
        {
            foreach ( $edges as $edge )
            {
                $code .= sprintf( "  node_%s -> node_%s [label=\"%s\" style=\"%s\"%s];\n", $this->dotize( $id ),
                    $this->dotize( $edge[ 'to' ] ), $edge[ 'name' ], $edge[ 'required' ] ? 'filled' : 'dashed',
                    $edge[ 'lazy' ] ? ' color="#9999ff"' : '' );
            }
        }
    ...
    private function findNodes(): array
    {
        ...
        foreach ( $container->getServiceIds() as $id )
        {
            ...
            {
                $nodes[ $id ] = [
                    'class' => str_replace( '\\', '\\\\', \get_class( $container->get( $id ) ) ),
