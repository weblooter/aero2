...
trait AddTrait
{
    ...
    final public function add( string $name, $path ): RouteConfigurator
    {
        ...
        if ( \is_array( $path ) )
        {
            ...
            {
                throw new \LogicException( sprintf( 'Route "%s" is missing routes for locale(s) "%s".', $name,
            ...
            else
            {
                foreach ( $path as $locale => $localePath )
                {
                    ...
                    {
                        throw new \LogicException( sprintf( 'Route "%s" with locale "%s" is missing a corresponding prefix in its parent collection.',
                            $name, $locale ) );
                    }
