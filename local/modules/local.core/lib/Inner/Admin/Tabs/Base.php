...
abstract class Base
{
    ...
    protected static function Initialize()
    {
        ...
        if (
        in_array(static::GetCurPage(), static::PAGES)
        ) {
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if ($ENTITY_ID = static::GetEntityId($_REQUEST[static::REQUEST_PARAM_NAME])) {
                    ...
                    if (
                    $USER_FIELD_MANAGER->CheckFields(static::UF_PROPERTY_ID, $ENTITY_ID, $arUfFields)
                    ) {
    ...
    public static function OnBeforeLocalRedirect(&$url, $skip_security_check, $bExternal)
    {
        foreach ($_REQUEST as $k => $v) {
            if (
                strpos($k, "active_tab") > 0
                && strpos($v, "user_fields") === 0
            ) {
                foreach (static::PAGES as $page) {
                    if (
                        strpos($url, $page) === 0
