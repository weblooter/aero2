...
class AdminEdit extends \Local\Core\Inner\AdminHelper\EditBase
{
    ...
    protected function setData()
    {
        ...
        if ($request->get("id") !== null) {
            try {
                ...
                $body = \Local\Core\Model\Data\BalanceLogTable::getList([
                    "select" => [
                        "*",
                    ],
                    "filter" => [
        ...
        if (!empty($body)) {
            $this->id = $body["ID"];
            $this->data = $body;
            $this->app->SetTitle("Редактирование лог баланса");
        } else {
            $rightAdd = $this->checkRights("can_add");
            if ($rightAdd->isSuccess()) {
                $this->app->SetTitle("Создание лога баланса");
            } else {
    ...
    protected function getTabsContent()
    {
        ...
        $columnName = [];
        $map = \Local\Core\Model\Data\BalanceLogTable::getMap();
        ...
        if ((int)$id > 0) {
            ...
            */
            ...
            $arFields = [
                ...
                'OPERATION' => trim($request->getPost('OPERATION')),
                'NOTE' => trim($request->getPost('NOTE'))
            ];

            $rightAdd = $this->checkRights("can_add");
            if ($rightAdd->isSuccess()) {
                try {
                    $res = \Local\Core\Model\Data\BalanceLogTable::add($arFields);
                    if ($res->isSuccess()) {
                        $this->id = $res->getId();
                    } else {
                        $result->addErrors($res->getErrors());
    ...
    protected function deleteAction(\Bitrix\Main\HttpRequest $request)
    {
        ...
        if ($this->id) {
            ...
            if ($rightDelete->isSuccess()) {
                try {
                    $res = \Local\Core\Model\Data\BalanceLogTable::delete((int)$this->id);
                    if (!$res->isSuccess()) {
                        $result->addErrors($res->getErrors());
                    }
                } catch (\Exception $e) {
