...
class AdminEdit extends \Local\Core\Inner\AdminHelper\EditBase
{
    ...
    protected function getTabsContent()
    {
        ...
        $canEdit = ( ( (int)$this->id > 0 && $this->checkRights("can_edit")->isSuccess() ) || ( (int)$this->id == 0 && $this->checkRights("can_add")->isSuccess() ) ) ? true : false;
        ...
        return [
            "main" => [
                ...
                ( new \Local\Core\Inner\AdminHelper\EditField\User($columnName['USER_OWN_ID'], 'USER_OWN_ID') )->setEditable($canEdit)->setRequired(true),
                ...
                ( new \Local\Core\Inner\AdminHelper\EditField\Select($columnName["VERIFIED"], "VERIFIED") )->setEditable($canEdit)->setVariants(self::$verified_values),

                ( new \Local\Core\Inner\AdminHelper\EditField\Textarea($columnName['VERIFIED_NOTE'], 'VERIFIED_NOTE') )->setEditable($canEdit)->setRequired(true),
            ],

            'ur_data' => [
                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_INN'], 'COMPANY_INN') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_NAME_SHORT'], 'COMPANY_NAME_SHORT') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_NAME_FULL'], 'COMPANY_NAME_FULL') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_OGRN'], 'COMPANY_OGRN') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_KPP'], 'COMPANY_KPP') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_OKPO'], 'COMPANY_OKPO') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_OKTMO'], 'COMPANY_OKTMO') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_DIRECTOR'], 'COMPANY_DIRECTOR') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_ACCOUNTANT'], 'COMPANY_ACCOUNTANT') )->setEditable($canEdit)->setRequired(true),
            ],

            'ur_address' => [
                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_ADDRESS_COUNTRY'], 'COMPANY_ADDRESS_COUNTRY') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_ADDRESS_REGION'], 'COMPANY_ADDRESS_REGION') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_ADDRESS_AREA'], 'COMPANY_ADDRESS_AREA') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_ADDRESS_CITY'], 'COMPANY_ADDRESS_CITY') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_ADDRESS_ADDRESS'], 'COMPANY_ADDRESS_ADDRESS') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_ADDRESS_OFFICE'], 'COMPANY_ADDRESS_OFFICE') )->setEditable($canEdit)->setRequired(true),

                ( new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['COMPANY_ADDRESS_ZIP'], 'COMPANY_ADDRESS_ZIP') )->setEditable($canEdit)->setRequired(true),
            ]
        ];
    }
