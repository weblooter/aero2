...
class AdminList extends \Local\Core\Inner\AdminHelper\ListBase
{
    ...
    protected function getFilterSearchFields()
    {
        return [
            ...
            "ACTIVE"        => [
                ...
                "TYPE"     => "SELECT",
                "VARIANTS" => SiteTable::getEnumFieldHtmlValues('ACTIVE')
            ],
            "DATE_CREATE"   => [
            ...
            "RESOURCE_TYPE"   => [
                ...
                "TYPE"     => "SELECT",
                "VARIANTS" => SiteTable::getEnumFieldHtmlValues('RESOURCE_TYPE')
            ],
            "FILE_LINK"       => [
            ...
            "HTTP_AUTH"       => [
                ...
                "TYPE"     => "SELECT",
                "VARIANTS" => SiteTable::getEnumFieldHtmlValues('HTTP_AUTH')
            ],
            "HTTP_AUTH_LOGIN" => [
        ...
        foreach( $filterSearch ?? [] as $code => $value )
        {
            ...
            if( !empty(trim($value)) )
            {
                switch( $code )
                {
                    ...
                    case "RESOURCE_TYPE":
                        if( in_array($value, SiteTable::getEnumFieldValues('RESOURCE_TYPE')) )
                        {
                            $arFilter["=".$code] = trim($value);
                        }
                        break;
    ...
    {
        return \Local\Core\Model\Data\SiteTable::getlist(
            [
                "select" => [
                    "*",
    ...
    protected function prepareRowField(\CAdminListRow $row, $fields)
    {
        ...
        $row->AddViewField(
            "RESOURCE_TYPE",
            SiteTable::getEnumFieldHtmlValues('RESOURCE_TYPE')[$fields["RESOURCE_TYPE"]]
        );
        $row->AddViewField(
            "HTTP_AUTH",
            SiteTable::getEnumFieldHtmlValues('HTTP_AUTH')[$fields["HTTP_AUTH"]]
        );
    }
    ...
    protected function deleteAction($id)
    {
        ...
        if( $checkRights->isSuccess() )
        {
            try
            {
                ...
                $res = \Local\Core\Model\Data\SiteTable::delete((int)$id);
                if( !$res->isSuccess() )
                {
                    $result->addErrors($res->getErrors());
                }
    ...
    {
        foreach( \Local\Core\Model\Data\SiteTable::getMap() ?? [] as $column )
        {
