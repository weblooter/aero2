    ...
    protected function getTabsContent()
    {
        ...
        return [
            "main" => [
                ...
                (new \Local\Core\Inner\AdminHelper\EditField\Select($columnName['GROUP'], 'GROUP'))->setVariants(MeasureTable::getEnumFieldHtmlValues('GROUP'))
                    ->setEditable($canEdit)
                    ->setRequired(true),
    ...
    protected function editAction(\Bitrix\Main\HttpRequest $request)
    {
        ...
        if ((int)$id > 0) {
            ...
            if ($rightEdit->isSuccess()) {
                try {
                    $res = \Local\Core\Model\Reference\MeasureTable::update($id, $arFields);
                    if (!$res->isSuccess()) {
