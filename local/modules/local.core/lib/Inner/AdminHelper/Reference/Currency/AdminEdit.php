...
class AdminEdit extends \Local\Core\Inner\AdminHelper\EditBase
{
    ...
    protected function getTabsContent()
    {
        ...
        $columnName = [];
        $map = \Local\Core\Model\Reference\CurrencyTable::getMap();
        ...
        return [
            "main" => [
                ...
                (new \Local\Core\Inner\AdminHelper\EditField\Text($columnName['NUMBER_OF_CURRENCY'], 'NUMBER_OF_CURRENCY'))->setEditable($canEdit)
                    ->setRequired(true),
            ],
        ];
    }
    ...
    protected function editAction(\Bitrix\Main\HttpRequest $request)
    {
        ...
        $arFields = [
            ...
            'NUMBER_OF_CURRENCY' => trim($request->getPost('NUMBER_OF_CURRENCY')),
        ];
        ...
        if ((int)$id > 0) {
            ...
            if ($rightAdd->isSuccess()) {
                try {
                    $res = \Local\Core\Model\Reference\CurrencyTable::add($arFields);
                    if ($res->isSuccess()) {
                        $this->id = $res->getId();
                    } else {
                        $result->addErrors($res->getErrors());
    ...
    protected function deleteAction(\Bitrix\Main\HttpRequest $request)
    {
        ...
        if ($this->id) {
            ...
            if ($rightDelete->isSuccess()) {
                try {
                    $res = \Local\Core\Model\Reference\CurrencyTable::delete((int)$this->id);
                    if (!$res->isSuccess()) {
                        $result->addErrors($res->getErrors());
                    }
                } catch (\Exception $e) {
