...
class CustomStepDataTree extends Base
{
    ...
    public function getRowHtml()
    {
        ...
        {
            $this->strHtml .= '<td class="adm-detail-content-cell-l">'.( $this->isRequired() ? '<b>' : '' ).$this->getTitle().( $this->isRequired() ? '</b>' : '' ).'</td><td>';
        }
        ...
        try
        {
            ...
            {
                $methodName = 'showTree'.mb_strtoupper(preg_replace('/[^a-zA-Z]/', '', $this->getValue()['TYPE']));
    ...
    private function SelectBoxFromArray($code, $ar, $val, $defaultText = '', $setReload = true, $isMultiple = false)
    {
        if( !$isMultiple )
        {
            if( $setReload )
            {
                $this->strHtml .= str_replace('<select ', '<select onChange="wblReloadCustomStepCondition.reloadBlock(\''.$this->codeFormated.'\')" ', SelectBoxFromArray($code, [
                                'reference' => array_values($ar),
                                'reference_id' => array_keys($ar)
                            ], $val, $defaultText)).' ';
            }
            else
            {
        ...
        else
        {
            if( $setReload )
            {
                $this->strHtml .= str_replace('<select ', '<select onChange="wblReloadCustomStepCondition.reloadBlock(\''.$this->codeFormated.'\')" style="width: auto;" ', SelectBoxMFromArray($code.'[]', [
                                'reference' => array_values($ar),
                                'reference_id' => array_keys($ar)
                            ], $val, $defaultText)).' ';
            }
            ...
            {
                $this->strHtml .= str_replace('<select ', '<select style="width: auto;" ', SelectBoxMFromArray($code.'[]', [
                                'reference' => array_values($ar),
                                'reference_id' => array_keys($ar)
                            ], $val, $defaultText)).' ';
            }
    ...
    {
        $this->SelectBoxFromArray($this->getCode().'[TYPE]', [
                'SIMPLE' => 'Свое значение',
                'RESOURCE' => 'Источник данных',
                'LOGIC_CONDITION' => 'Сложное условие'
            ], $this->getValue()['TYPE'] ?? '', "-- Выберите тип значения --");
    }
    ...
    {
        $this->SelectBoxFromArray($this->getCode().'[RESOURCE]', [
                'CATALOG' => 'Каталог',
                'OFFER' => 'Торговое предложение'
            ], $this->getValue()['RESOURCE'] ?? '', "-- Выберите источник данных --");
        ...
        if( !empty($this->getValue()['RESOURCE']) )
        {
            switch( $this->getValue()['RESOURCE'] )
            {
                ...
                case 'CATALOG': // Каталог
                    ...
                    $this->SelectBoxFromArray($this->getCode().'[VALUE]', $this->getResourceIblockFieldsAndProps('catalog'), $this->getValue()['VALUE'] ?? '', "-- Выберите значение --", false, $this->isMultiple);
                ...
                case 'OFFER': // Торг. предложение
                    ...
                    $this->SelectBoxFromArray($this->getCode().'[VALUE]', $this->getResourceIblockFieldsAndProps('offers'), $this->getValue()['VALUE'] ?? '', "-- Выберите значение --", false, $this->isMultiple);
    ...
    private function showTreeLOGICCONDITION()
    {
        ...
        foreach( $arValues["CONDITIONS"] as $key => $arValue )
        {
            ...
            if( !empty($arValue["CONDITION"]) && is_array($arValue["CONDITION"]) )
            {
                ...
                $obCondValue->Init(BT_COND_MODE_GENERATE, BT_COND_BUILD_CATALOG);
    ...
    private function getResource($inputCode, $inputValue)
    {
        ...
        $this->SelectBoxFromArray($inputCode.'[TYPE]', [
            'SIMPLE' => 'Свое значение',
            'RESOURCE' => 'Источник данных'
        ], $inputValue['TYPE'] ?? '', "-- Выберите тип значения --");
        ...
        if( !empty($inputValue['TYPE']) )
        {
            switch( $inputValue['TYPE'] )
            {
                ...
                case "RESOURCE":
                    ...
                    $this->SelectBoxFromArray($inputCode.'[RESOURCE]', [
                        'CATALOG' => 'Каталог',
                        'OFFER' => 'Торговое предложение'
                    ], $inputValue['RESOURCE'] ?? '', "-- Выберите источник данных --");
                    ...
                    if( !empty($inputValue['RESOURCE']) )
                    {
                        switch( $inputValue['RESOURCE'] )
                        {
                            // Каталог
                            case 'CATALOG':
                                $this->SelectBoxFromArray($inputCode.'[VALUE]', $this->getResourceIblockFieldsAndProps('catalog'), $inputValue['VALUE'] ?? '', "-- Выберите значение --", true, $this->isMultiple);
                                break;
                            // Торг. предложение
                            case 'OFFER':
                                $this->SelectBoxFromArray($inputCode.'[VALUE]', $this->getResourceIblockFieldsAndProps('offers'), $inputValue['VALUE'] ?? '', "-- Выберите значение --", true, $this->isMultiple);
                                break;
                        }
                    }
                    break;
