...
class XMLReader extends \Local\Core\Inner\Robofeed\XMLReader\AbstractXMLReader
{
    ...
    public function run()
    {
        ...
        $this->getImporter()->setStoreId($this->intStoreId);
        ...
        {
            $this->obResult->addError( new \Bitrix\Main\Error('Во время проверки робофид XML произошла критическая ошибка. Убедитесь, что в Вашем робофиде XML нет ошибки, а теги закрыты. Самый быстрый способ найти ошибку - откройте робофид XML в браузере.') );
        }
        $this->obReader->close();
    ...
    protected function callDefaultValuesOffer($reader)
    {
        ...
        $obDefaultValuesOfferValidateResult = $this->getValidator()->validateDefaultValuesOffer($obXml, $this->arSchema['robofeed']['defaultValues']['offer']);
    ...
    protected function callCategories($reader)
    {
        ...
        $obCategoriesValidateResult = $this->getValidator()->validateCategories($obXml, $this->arSchema['robofeed']['categories']['category']);
        ...
        {
            $this->getImporter()->importCategories($this->arBodyValues['categories']['category']);
        }
    ...
    protected function callOffer($reader)
    {
        ...
        $obOfferValidateResult = $this->getValidator()->validateOffer(
            $obXml,
        ...
        switch($this->constScript)
        {
            ...
            case self::SCRIPT_IMPORT:
                ...
                dump($arOfferFields);
                $this->getImporter()->importOffer($arOfferFields);
