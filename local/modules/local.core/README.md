...
### \Local\Core\Model\Robofeed
**ORM сущности, которые являются в модуле по работе с Robofeed.** Описывать долго, нудно, найхуй нужно. Туда все равно никто лезть до 2й версии Робофида не будет.
...
---
### \Local\Core\Inner\TradingPlatform\
Это область для работы с ТП - обработчики, ежемесячные оплаты, создание экспорта. Все, что связано с ТП хранится там. Содержит свой **README.MD**, с которым необходимо ознакомиться.

---
### Local\Core\Inner\Robofeed\
Это область для работы с робофидом - обработчики версий, карта, импорт и т.п. Содержит все, что так или иначе связано с импортом робофида.Содержит свой **README.MD**, с которым необходимо ознакомиться.
